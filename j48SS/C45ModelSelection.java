...
public class C45ModelSelection extends ModelSelection {
    /** for serialization */
    private static final long serialVersionUID = 7712232815704099639L;
    ...
    public C45ModelSelection(
            ...
            double minimumSupport, boolean useIGPruning, int maxGap, int maxPatLength,
            double patternWeight, int popSize, int numEvals, double crossoverP, double mutationP,
            boolean isVerbose) {
        m_minNoObj                       = minNoObj;
        ...
        try {
            ...
            for (int i = 0; i < data.numAttributes(); i++) {
                if (i != data.classIndex() && ! data.attribute(i).isString()) {
                    ...
                    if (currentModel[i].checkModel() &&
                            Utils.gr(currentModel[i].infoGain() / Utils.log2(currentModel[i].splitNumArgs()),
                                    minResult)) {
                        bestModel = currentModel[i];
                        minResult = currentModel[i].infoGain() / Utils.log2(currentModel[i].splitNumArgs());
                    }
                }
            }
            ...
            for (int i = 0; i < minResultOneVsAll.length; i++) {
                if (bestModel == null) {
                    ...
                    minResultOneVsAll[i] = bestModel.getOneVsAllInfoGain(i);
                }
                else {
                    minResultOneVsAll[i] = 0.0D;
                }
            ...
            for (int index : sequentialAttIndices) {
                currentModel[index] = new C45Split(index, m_minNoObj, sumOfWeights, m_useMDLcorrection,
            ...
            for (int index : tsAttIndices) {
                currentModel[index] = new C45Split(index, m_minNoObj, sumOfWeights, m_useMDLcorrection, m_popSize,
            ...
            // Add all Instances with unknown values for the corresponding attribute to the distribution for the
            // model, so
